...
export const SettingsDialog: FC<SettingsDialogProps> = ({ open, onOpenChange }) => {
  ...
  const renameThread = async (id: string, currentTitle: string) => {
    ...
    await fetch("/api/threads", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, title: newTitle }) });
    setThreads((t) => t.map((x) => (x.id === id ? { ...x, title: newTitle } : x)));
  };
  ...
  const exportThread = async (id: string, title: string) => {
    ...
    a.href = url; a.download = `${title.replace(/[^a-zA-Z0-9]/g, "_")}.json`; a.click();
            ...
            <div className="flex gap-1">
              {(["general", "history"] as const).map((t) => (
                <button key={t} type="button" onClick={() => setTab(t)}
                  className={cn("px-3 py-1.5 font-medium text-sm transition-colors", tab === t ? "border-primary border-b-2 text-primary" : "text-muted-foreground hover:text-foreground")}>
                  {t === "general" ? "Instructions" : "Chat History"}
                </button>
              ))}
            </div>
            <button type="button" onClick={() => setShowJson(true)} className="rounded p-1.5 text-muted-foreground hover:bg-muted" title="View JSON Config">
            ...
            <div className="flex flex-col gap-4 pt-1">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">Custom instructions for the AI</p>
                </div>
                <button type="button" role="switch" aria-checked={enabled} onClick={() => setEnabled(!enabled)}
                  className={cn("relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors", enabled ? "bg-primary" : "bg-input")}>
                  <span className={cn("pointer-events-none block size-5 rounded-full bg-background shadow-lg ring-0 transition-transform", enabled ? "translate-x-5" : "translate-x-0")} />
                </button>
              </div>

              {enabled && (
                <>
                  {/* Preset buttons — Grok style */}
                  <div className="flex flex-wrap gap-1.5">
                    <button type="button"
                      onClick={() => setActivePreset(null)}
                      className={cn("flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-medium transition-all",
                        activePresetId === null ? "border-primary bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted")}>
                      Custom
                      {activePresetId === null && <CheckIcon className="size-3" />}
                    </button>
                    {presets.map((p) => (
                      <div key={p.id} className="group relative flex items-center">
                        <button type="button"
                          ...
                          className={cn("flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-medium transition-all",
                            activePresetId === p.id ? "border-2 shadow-sm" : "text-muted-foreground hover:bg-muted")}
                          style={activePresetId === p.id ? { borderColor: p.color, backgroundColor: `${p.color}15`, color: p.color } : undefined}>
                          {p.name}
                          {activePresetId === p.id && <CheckIcon className="size-3" />}
                        </button>
                        {!p.builtin && (
                          <button type="button" onClick={() => deletePreset(p.id)}
                            className="ml-0.5 hidden rounded-full p-0.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive group-hover:block">
                    ...
                    {!creating && (
                      <button type="button" onClick={() => setCreating(true)}
                        className="flex items-center gap-1 rounded-full border border-dashed px-3 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted">
                    ...
                    <div className="flex flex-col gap-2 rounded-lg border border-dashed p-3">
                      ...
                      <div className="flex justify-end gap-2">
                        ...
                        <Button size="sm" disabled={!newName.trim()} onClick={() => {
                          addPreset({ name: newName, prompt: newPrompt, color: newColor });
                          setCreating(false); setNewName(""); setNewPrompt(""); setNewColor("#10b981");
                        }}>Create</Button>
                      </div>
                    </div>
                  )}

                  {/* Textarea — shows preset prompt (readonly) or custom prompt (editable) */}
                  {activePresetId ? (
                    <div className="relative">
                      <textarea
                        value={presets.find((p) => p.id === activePresetId)?.prompt || ""}
                        readOnly
                        className="min-h-28 w-full resize-y rounded-lg border bg-muted/50 px-3 py-2 text-sm text-muted-foreground outline-none"
                        rows={4}
                      />
                    ...
                    <textarea
                      ...
                      placeholder="How should Skyler behave?"
                      className="min-h-28 w-full resize-y rounded-lg border bg-background px-3 py-2 text-sm outline-none placeholder:text-muted-foreground/60 focus:border-ring/75 focus:ring-2 focus:ring-ring/20"
                      rows={4}
                    />
                  )}
                </>
            ...
            <div className="flex flex-col gap-2 pt-2">
              ...
              <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-dashed px-3 py-2 text-muted-foreground text-sm transition-colors hover:bg-muted">
                ...
                <input type="file" accept=".json" className="hidden" onChange={async (e) => {
                  ...
                  try { const text = await file.text(); const data = JSON.parse(text); if (data.id && data.messages) { await fetch("/api/threads", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }); loadThreads(); } } catch {}
                  e.target.value = "";
                }} />
              </label>
            </div>
